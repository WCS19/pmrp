"""Property tests for the RISK-012 strategy-capital limit rule."""
...
MARKET_ID = MarketId("mkt_risk_strategy_capital_property")
CONTRACT_ID = ContractId("ctr_risk_strategy_capital_property")
STRATEGY_ID = StrategyId("strat_risk_strategy_capital_property")
CENT = Decimal("0.01")
...
async def test_strategy_capital_limit_rule_is_deterministic_at_exact_cap(
    current_dollars: int,
    quantity: int,
    price_cents: int,
) -> None:
    ...
    max_strategy_capital = Decimal(current_dollars) + order_capital
    rule = StrategyCapitalLimitRule(max_state_age=timedelta(milliseconds=5000))
    context = _context(
    ...
    assert first.reason_code == RISK_STRATEGY_CAPITAL_VALID_REASON
    assert first.observed_value == max_strategy_capital
    assert first.limit_value == max_strategy_capital
...
async def test_strategy_capital_limit_rule_rejects_any_capital_above_cap(
    current_dollars: int,
    quantity: int,
    price_cents: int,
) -> None:
    ...
    result = await StrategyCapitalLimitRule(max_state_age=timedelta(milliseconds=5000)).evaluate(
        _intent(quantity=str(quantity), limit_price=str(price)),
        _context(
            current_capital_used=Decimal(current_dollars),
            max_strategy_capital=cap,
            observed_at=NOW,
        ),
    )
...
async def test_strategy_capital_limit_rule_includes_pending_capital_at_exact_cap(
    current_dollars: int,
    ...
    result = await StrategyCapitalLimitRule(max_state_age=timedelta(milliseconds=5000)).evaluate(
        ...
        _context(
            ...
            max_strategy_capital=projected_capital,
            observed_at=NOW,
        ),
    )
...
async def test_strategy_capital_limit_rule_rejects_any_stale_capital_state(age_ms: int) -> None:
    result = await StrategyCapitalLimitRule(max_state_age=timedelta(milliseconds=5000)).evaluate(
...
async def test_strategy_capital_limit_rule_rejects_any_future_capital_state(
    future_ms: int,
) -> None:
    result = await StrategyCapitalLimitRule(max_state_age=timedelta(milliseconds=5000)).evaluate(
        _intent(quantity="10", limit_price="0.50"),
...
def _intent(*, quantity: str, limit_price: str) -> OrderIntent:
    return OrderIntent.model_validate(
        {
            "intent_id": "intent_risk_strategy_capital_property",
            ...
            "signal_ids": (),
            "correlation_id": "corr_risk_strategy_capital_property",
            "idempotency_key": "idem-risk-strategy-capital-property",
        }
    )
