"""Property tests for the RISK-010 portfolio gross-exposure limit rule."""
...
MARKET_ID = MarketId("mkt_risk_portfolio_gross_property")
CONTRACT_ID = ContractId("ctr_risk_portfolio_gross_property")
STRATEGY_ID = StrategyId("strat_risk_portfolio_gross_property")
CENT = Decimal("0.01")
...
async def test_portfolio_gross_limit_rule_is_deterministic_at_exact_cap(
    current_dollars: int,
    quantity: int,
    price_cents: int,
) -> None:
    price = Decimal(price_cents) / Decimal("100")
    order_exposure = Decimal(quantity) * price
    max_gross_exposure = Decimal(current_dollars) + order_exposure
    rule = PortfolioGrossLimitRule(max_state_age=timedelta(milliseconds=5000))
    context = _context(
        current_gross_exposure=Decimal(current_dollars),
    ...
    assert first.reason_code == RISK_PORTFOLIO_GROSS_VALID_REASON
    assert first.observed_value == max_gross_exposure
    assert first.limit_value == max_gross_exposure
...
async def test_portfolio_gross_limit_rule_rejects_any_exposure_above_cap(
    current_dollars: int,
    quantity: int,
    price_cents: int,
) -> None:
    ...
    result = await PortfolioGrossLimitRule(max_state_age=timedelta(milliseconds=5000)).evaluate(
...
async def test_portfolio_gross_limit_rule_includes_pending_exposure_at_exact_cap(
    current_dollars: int,
    ...
    result = await PortfolioGrossLimitRule(max_state_age=timedelta(milliseconds=5000)).evaluate(
        ...
        _context(
            current_gross_exposure=Decimal(current_dollars),
            open_order_gross_exposure=Decimal(open_order_dollars),
            reserved_gross_exposure=Decimal(reserved_dollars),
            max_gross_exposure=projected_exposure,
            observed_at=NOW,
        ),
    )
    ...
    assert result.observed_value == projected_exposure
    assert result.limit_value == projected_exposure


@given(age_ms=st.integers(min_value=5001, max_value=600000))
async def test_portfolio_gross_limit_rule_rejects_any_stale_exposure_state(age_ms: int) -> None:
    result = await PortfolioGrossLimitRule(max_state_age=timedelta(milliseconds=5000)).evaluate(
...
async def test_portfolio_gross_limit_rule_rejects_any_future_exposure_state(
    future_ms: int,
) -> None:
    result = await PortfolioGrossLimitRule(max_state_age=timedelta(milliseconds=5000)).evaluate(
        _intent(quantity="10", limit_price="0.50"),
    ...
    *,
    ...
    return RiskContext(
        ...
        portfolio_gross_exposure=RiskPortfolioGrossExposureState(
            ...
            open_order_gross_exposure=open_order_gross_exposure,
...
def _intent(*, quantity: str, limit_price: str) -> OrderIntent:
    return OrderIntent.model_validate(
        {
            "intent_id": "intent_risk_portfolio_gross_property",
            ...
            "signal_ids": (),
            "correlation_id": "corr_risk_portfolio_gross_property",
            "idempotency_key": "idem-risk-portfolio-gross-property",
        }
    )
