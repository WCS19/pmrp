"""Property tests for the RISK-003 market-enabled rule."""
...
MARKET_ID = MarketId("mkt_risk_market_enabled_property")
STRATEGY_ID = StrategyId("strat_risk_market_enabled_property")


@given(enabled=st.booleans(), age_ms=st.integers(min_value=0, max_value=5000))
async def test_market_enabled_rule_is_deterministic_with_fresh_state(
    enabled: bool,
    age_ms: int,
) -> None:
    rule = MarketEnabledRule(max_state_age=timedelta(milliseconds=5000))
    context = RiskContext(
        evaluated_at=NOW,
        market_enabled={
    ...
    assert first.reason_code == (
        RISK_MARKET_ENABLED_REASON if enabled else RISK_MARKET_DISABLED_REASON
    )
...
async def test_market_enabled_rule_rejects_any_stale_state(age_ms: int) -> None:
    result = await MarketEnabledRule(max_state_age=timedelta(milliseconds=5000)).evaluate(
        _intent(),
        RiskContext(
            evaluated_at=NOW,
            market_enabled={
...
async def test_market_enabled_rule_rejects_any_future_state(future_ms: int) -> None:
    result = await MarketEnabledRule(max_state_age=timedelta(milliseconds=5000)).evaluate(
        _intent(),
        RiskContext(
            evaluated_at=NOW,
...
def _intent() -> OrderIntent:
    return OrderIntent.model_validate(
        {
            "intent_id": "intent_risk_market_enabled_property",
            "strategy_id": STRATEGY_ID,
            "market_id": MARKET_ID,
            "contract_id": "ctr_risk_market_enabled_property",
            "outcome_id": "out_yes",
            "side": Side.BUY,
            "quantity": "10",
            "limit_price": "0.43",
            ...
            "signal_ids": (),
            "correlation_id": "corr_risk_market_enabled_property",
            "idempotency_key": "idem-risk-market-enabled-property",
        }
    )
