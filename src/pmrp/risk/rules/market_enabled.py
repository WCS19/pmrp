...
RISK_MARKET_ENABLED_RULE_ID = "RISK-003"
RISK_MARKET_ENABLED_RULE_VERSION = "1.0"
RISK_MARKET_ENABLED_REASON = "RISK_MARKET_ENABLED"
RISK_MARKET_DISABLED_REASON = "RISK_MARKET_DISABLED"
RISK_MARKET_STATE_MISSING_REASON = "RISK_MARKET_STATE_MISSING"
RISK_MARKET_STATE_STALE_REASON = "RISK_MARKET_STATE_STALE"
RISK_MARKET_STATE_FUTURE_REASON = "RISK_MARKET_STATE_FUTURE"
...
class MarketEnabledRule:
    """Reject intents for disabled, missing, stale, or future market state."""
    ...
    def rule_id(self) -> str:
        ...
        return RISK_MARKET_ENABLED_RULE_ID

    @property
    def version(self) -> str:
        """Return the stable risk rule version."""

        return RISK_MARKET_ENABLED_RULE_VERSION

    async def evaluate(self, intent: OrderIntent, context: RiskContext) -> RiskRuleResult:
        """Evaluate whether an intent's market is enabled in the risk snapshot."""

        if not isinstance(intent, OrderIntent):
            raise RiskInputError("market enabled rule requires an OrderIntent")
        if not isinstance(context, RiskContext):
            raise RiskInputError("market enabled rule requires a RiskContext")

        state = context.market_enabled_state(intent.market_id)
        if state is None:
            return self._result(
                passed=False,
                reason_code=RISK_MARKET_STATE_MISSING_REASON,
                observed_value=None,
                limit_value=_ONE,
                unit=_ENABLED_UNIT,
        ...
        if state.observed_at > context.evaluated_at:
            return self._result(
                passed=False,
                reason_code=RISK_MARKET_STATE_FUTURE_REASON,
        ...
        if age > self.max_state_age:
            return self._result(
                passed=False,
                reason_code=RISK_MARKET_STATE_STALE_REASON,
        ...
        return self._result(
            passed=state.value,
            reason_code=RISK_MARKET_ENABLED_REASON if state.value else RISK_MARKET_DISABLED_REASON,
